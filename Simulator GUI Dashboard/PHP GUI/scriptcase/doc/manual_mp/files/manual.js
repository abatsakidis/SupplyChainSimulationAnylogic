...
{
	var folder = document.location.href.split('manual_mp.htm')[0];
	window.history.pushState("Scriptcase Web Help", "Scriptcase Web Help", folder + 'manual_mp.htm?item='+page);
	if(document.location.href.indexOf(folder + 'manual_mp.htm?item='+page) < 0)
	{
		document.location.href = folder + 'manual_mp.htm?item='+page;
	}
}
...
function changeInternalPage(page)
{
	...
	if(page == 'home')
	{
			$('#id_bodyContent').attr('src', document.location.origin + (document.location.pathname).replace(/\\/g, "/").replace(/\/[^\/]*\/?$/, "") + '/manual_mpbody.htm');
	}
