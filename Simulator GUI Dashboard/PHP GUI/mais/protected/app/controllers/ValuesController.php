...
class ValuesController extends BaseController {
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','Values.index',$this->data);
	}		
	...
	{ 
		...
		return View::make('Values.table',$this->data);
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('tb_values'); 
		}
		...
		$this->data['id'] = $id;
		return View::make('Values.form',$this->data);
	...
	function getShow( $id = null)
	{
		...
		return View::make('Values.view',$this->data);
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('tb_values');
	...
	function postCopy()
	{
	    ...
	    foreach(DB::select("SHOW COLUMNS FROM tb_values") as $column)
        {
			if( $column->Field != 'id')
		...
		$sql = "INSERT INTO tb_values (".implode(",", $columns).") ";
		$sql .= " SELECT ".implode(",", $columns)." FROM tb_values WHERE id IN (".$toCopy.")";
		DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
