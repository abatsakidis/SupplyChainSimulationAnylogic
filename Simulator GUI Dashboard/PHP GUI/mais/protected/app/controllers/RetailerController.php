...
class RetailerController extends BaseController {
	...
	public $module = 'Retailer';
	static $per_page	= '10';
	
	public function __construct() {
		parent::__construct();
		$this->model = new Retailer();
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','Retailer.index',$this->data);
	}		
	...
	{ 
		...
		return View::make('Retailer.table',$this->data);
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('retailer'); 
		}
		...
		$this->data['id'] = $id;
		return View::make('Retailer.form',$this->data);
	...
	function getShow( $id = null)
	{
		...
		return View::make('Retailer.view',$this->data);
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('retailer');
	...
	function postCopy()
	{
	    ...
	    foreach(DB::select("SHOW COLUMNS FROM retailer") as $column)
        {
			if( $column->Field != 'id')
		...
		$sql = "INSERT INTO retailer (".implode(",", $columns).") ";
		$sql .= " SELECT ".implode(",", $columns)." FROM retailer WHERE id IN (".$toCopy.")";
		DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
