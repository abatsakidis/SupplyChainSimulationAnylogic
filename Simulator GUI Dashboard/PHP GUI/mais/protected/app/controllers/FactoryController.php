...
class FactoryController extends BaseController {
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','Factory.index',$this->data);
	}		
	...
	{ 
		...
		return View::make('Factory.table',$this->data);
	...
	function getAdd( $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('factory'); 
		}
		...
		$this->data['id'] = $id;
		return View::make('Factory.form',$this->data);
	...
	function getShow( $id = null)
	{
		...
		return View::make('Factory.view',$this->data);
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			$data = $this->validatePost('factory');
	...
	function postCopy()
	{
	    ...
	    foreach(DB::select("SHOW COLUMNS FROM factory") as $column)
        {
			if( $column->Field != 'id')
		...
		$sql = "INSERT INTO factory (".implode(",", $columns).") ";
		$sql .= " SELECT ".implode(",", $columns)." FROM factory WHERE id IN (".$toCopy.")";
		DB::insert($sql);
		return Response::json(array(
			'status'=>'success',
			'message'=> SiteHelpers::alert('success',Lang::get('core.note_success'))
