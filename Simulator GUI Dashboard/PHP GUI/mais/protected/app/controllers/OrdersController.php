...
class OrdersController extends BaseController {
	...
	public function getIndex()
	{
		...
		$this->layout->nest('content','Orders.index',$this->data)
	...
	function getAdd( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->layout->nest('content','Orders.form',$this->data)->with('menus', $this->menus );	
	}
	...
	function getShow( $id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['access']		= $this->access;
		$this->layout->nest('content','Orders.view',$this->data)->with('menus', $this->menus );	
	}	
	...
	function postCopy()
	{
		...
		DB::insert($sql);
		return Redirect::to('Orders')->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));		
	}	
	...
	function postSave( $id =0)
	{
		...
		if ($validator->passes()) {
			...
			$redirect = (!is_null(Input::get('apply')) ? 'Orders/add/'.$id.'?md='.$md.$trackUri :  'Orders?md='.$md.$trackUri );
			return Redirect::to($redirect)->with('message', SiteHelpers::alert('success',Lang::get('core.note_success')));
		} else {
			return Redirect::to('Orders/add/'.$id.'?md='.$md)->with('message', SiteHelpers::alert('error',Lang::get('core.note_error')))
	...
	public function postDestroy()
	{
		...
		return Redirect::to('Orders?md='.Input::get('md'));
	}			
