<?php usort($tableGrid, "SiteHelpers::_sort"); ?>
	@include( $pageModule.'/toolbar')
	 ...
	 <?php echo Form::open(array('url'=>'Products/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
    ...
    <table class="table table-striped  " id="{{ $pageModule }}Table">
        ...
        <tbody>
			<tr id="ProductsSearch"  >
				...
				<button type="button"  class=" do-quick-search btn btn-xs btn-info" onClick="ajaxDoSearch('#Products','<?php echo $pageUrl.'/data?md='.$masterdetail["filtermd"];?>')"><i class="fa fa-search"></i></button>
                ...
                <tr>
					...
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="<?php echo $row->ProductID ;?>" />  </td>									
				 ...
				 <td>
					...
					<?php $id = SiteHelpers::encryptID($row->ProductID);
				 	if($access['is_detail'] ==1) :?>
					<a href="<?php echo  URL::to('Products/show/'.$id);?>"  class="tips btn btn-xs btn-primary"  
					title="<?php echo  Lang::get('core.btn_view');?>"
					onclick="ajaxViewDetail('#{{ $pageModule }}',this.href); return false;"  >
					<i class="fa  fa-search"></i></a>
					<?php endif ;
					if($access['is_edit'] ==1) : ?>
					<a  href="<?php echo URL::to('Products/add/'.$id.'?md='.$masterdetail["filtermd"]);?>"  class="tips btn btn-xs btn-success"  
