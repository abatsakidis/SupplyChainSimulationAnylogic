...
<div class="sbox">
  ...
  {{ Form::open(array('url'=>$module.'/savesetting/'.$module_name, 'class'=>'form-horizontal ')) }}
  <input  type='text' name='module_id' id='module_id'  value='{{ $row->module_id }}'  style="display:none; " />
  	<fieldset>
		<legend> Module Setting </legend>
		  ...
		  <div class="form-group">
			...
			<div class="col-md-8">
				...
				@if($setting['gridtype'] =='native') checked="checked" @endif 
				 /> Native  
				</label>
				<label class="radio">
				<input type="radio" value="ajax" name="grid-type" 
				@if($setting['gridtype'] =='ajax') checked="checked" @endif 				
				/> Ajax  
				</label>							
			 </div> 
		  </div> 
		...
		<div class="form-group ">
			...
			<label class="checkbox">
				...
				@if($setting['frozen'] =='true') checked="checked" @endif 				
				/> Enable 
			</label>	
			</div>
		</div>	
