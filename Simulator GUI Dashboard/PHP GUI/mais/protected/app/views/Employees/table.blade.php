<?php usort($tableGrid, "SiteHelpers::_sort"); ?>
	@include( $pageModule.'/toolbar')
	 ...
	 <?php echo Form::open(array('url'=>'Employees/destroy/', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
    ...
    <table class="table table-striped  " id="{{ $pageModule }}Table">
        ...
        <tbody>
			<tr id="EmployeesSearch"  >
				...
				<button type="button"  class=" do-quick-search btn btn-xs btn-info" onClick="ajaxDoSearch('#Employees','<?php echo $pageUrl.'/data?md='.$masterdetail["filtermd"];?>')"><i class="fa fa-search"></i></button>
                ...
                <tr>
					...
					<td width="50"><input type="checkbox" class="ids" name="id[]" value="<?php echo $row->EmployeeID ;?>" />  </td>									
				 ...
				 <td>
					...
					<?php $id = SiteHelpers::encryptID($row->EmployeeID);
				 	if($access['is_detail'] ==1) :?>
					<a href="<?php echo  URL::to('Employees/show/'.$id);?>"  class="tips btn btn-xs btn-primary"  
					title="<?php echo  Lang::get('core.btn_view');?>"
					onclick="ajaxViewDetail('#{{ $pageModule }}',this.href); return false;"  >
					<i class="fa  fa-search"></i></a>
					<?php endif ;
					if($access['is_edit'] ==1) : ?>
					<a  href="<?php echo URL::to('Employees/add/'.$id.'?md='.$masterdetail["filtermd"]);?>"  class="tips btn btn-xs btn-success"  
